...
def build_blogger_xml(structure: PageStructure | dict, *, template_name: str = "simple") -> str:
    ...
    is_landing = bool(features.get("landing"))
...
"""
    ...
    landing_section = ""
    if is_landing:
        landing_title = escape(str(structure.get("title") or "Launch your blog"))
        landing_description = escape(
            str(structure.get("description") or "A focused landing page generated by BloggerEasy.")
        )
        headings = [
            str(item)
            for item in (structure.get("headings") or [])
            if str(item).strip() and str(item).strip() != str(structure.get("title") or "").strip()
        ][:3]
        paragraphs = [str(item) for item in (structure.get("sample_paragraphs") or []) if str(item).strip()]
        feature_cards = []
        for idx, label in enumerate((headings or ["Hero", "CTA", "Features"])[:3]):
            body = paragraphs[idx] if idx < len(paragraphs) else "A Blogger-ready landing section."
    ...
    <section class='landing-hero'>
      ...
      <a class='landing-cta' expr:href='data:blog.homepageUrl'>Get started</a>
    </section>
